...
class MST_Pdp_Block_Adminhtml_Artworkcate_Edit_Tab_Artwork extends Mage_Adminhtml_Block_Widget_Grid
{
	public function __construct()
	{
		parent::__construct();
		$this->setId('image_artGrid');
	...
	protected function _addColumnFilterToCollection($column)
	{
		...
		if ($column->getId() == 'in_products') {
			...
			if ($column->getFilter()->getValue()) {
				$this->getCollection()->addFieldToFilter('image_id', array('in'=>$ids));
			} else {
				if($ids) {
					$this->getCollection()->addFieldToFilter('image_id', array('nin'=>$ids));
				}
			}
		} else {
	...
	protected function _prepareColumns()
	{
		...
		$this->addColumn('filename', array(
			'header'    => Mage::helper('pdp')->__('Artwork'),
			'index'     => 'filename',
	...
	public function getSelectedImages()
	{
		...
		foreach($collection as $obj){
			$custIds[$obj->getImageId()] = array('position' => $obj->getPosition());
		}
		return $custIds;
	}
}