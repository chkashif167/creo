...
class Text_Renderer extends Abstract_Renderer {
  ...
  function render(Frame $frame) {
    ...
    if (strtolower($style->direction) == 'rtl') {
		$tmp_texts = explode(" ",$text);
		
		foreach($tmp_texts as $key=>$tmp_text){
			if(!mb_detect_encoding($tmp_text, 'ASCII', true)){
				preg_match_all('/./us', $tmp_text, $ar);
				$tmp_texts[$key] = join('',array_reverse($ar[0]));
			}
		}
		
		/*reverse only unicode word*/
		$tmpTextArray 	= array();
		$tmpReversedTextArray 	= array();
		$tmpTexts		= array();
		$text = '';
		foreach($tmp_texts as $tmpText){
			$reversedFlag = mb_detect_encoding($tmpText, 'ASCII', true)?false:true;/*False if is ASCII*/
			if(!$reversedFlag){
				if(sizeof($tmpReversedTextArray)) {
					$tmpTexts[] = join(' ',array_reverse($tmpReversedTextArray));
					$tmpReversedTextArray = null;
				}
				$tmpTextArray[] = $tmpText;
			}else{
				if(sizeof($tmpTextArray)) {
					$tmpTexts[] = join(' ',$tmpTextArray);
					$tmpTextArray = null;
				}
				$tmpReversedTextArray []= $tmpText;
			}
		}
		if(sizeof($tmpTextArray)) {
			$tmpTexts[] = join(' ',$tmpTextArray);
			$tmpTextArray = null;
		}else if(sizeof($tmpReversedTextArray)) {
			$tmpTexts[] = join(' ',array_reverse($tmpReversedTextArray));
			$tmpReversedTextArray = null;
		}
		$text = join(' ',array_reverse($tmpTexts));
