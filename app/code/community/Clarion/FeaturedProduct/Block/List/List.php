 ...
 * 
 */
class Clarion_FeaturedProduct_Block_List_List extends Mage_Catalog_Block_Product_Abstract
{  
    /**
    ...
    public function getNumberOfProduct()
    {
        if (null === $this->_numberOfProduct) {
            if($this->getFeaturedPrdoctsOnPage() == 'home_page'){
                $configNumberOfProduct = Mage::Helper('clarion_featuredproduct')->getNumberOfProductsHomePage();
            } else {
                $configNumberOfProduct = Mage::Helper('clarion_featuredproduct')->getNumberOfProductsCatPage();
            }
    ...
    {
       $pager = $this->getChild('featured-product.list.pager');
        if ($pager) {
