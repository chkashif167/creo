...
var TCO = new function() {
    ...
    this.requestToken = function(f, c) {
        var n = new TokenRequest();
        n.fillTokenRequest(f, c);
        if (validate(n, f)) {
            ...
            ajax2co({
                ...
                success: function (r, i, s) {
                  if (r.response !== null) {
                      ...
                      sendTokenRequest(c, n);
                  } else if (r.exception !== null) {
                      defaultErrorFunction(c, r.exception.errorMsg, r.exception.errorCode);
                  } else {
                      defaultErrorFunction(c);
                  }
                },
                error: function (e) {
                    defaultErrorFunction(c);
    ...
    var sendTokenRequest = function(c, n) {
        try {
			sellerId = n.sellerId;
    ...
    var validate = function(f, c) {
        var t = true;
        f.expMonth = (f.expMonth.toString().length === 1) ? '0'+f.expMonth : f.expMonth;
        f.expYear = (f.expYear.toString().length < 4) ? '20'+f.expYear : f.expYear;
        f.cardNum = f.cardNum.replace(/[^0-9]+/g,'');
        f.cvv = f.cvv.replace(/[^0-9]+/g,'');
        if (!isNumber(f.cardNum) || !isNumber(f.cvv) || !isNumber(f.expMonth) || !isNumber(f.expYear)) {
           t = false;
           defaultErrorFunction(c, "Missing Card Data", 401);
        } else if (typeof publicKey === 'undefined') {
           t = false;
           defaultErrorFunction(c, "Missing publicKey.js", 401);
        }
        return t;
    };
    ...
    var TokenRequest = function() {
        ...
        this.fillTokenRequest = function (f, c) {
        	if (f.sellerId && f.publishableKey && f.ccNo && f.expMonth && f.expYear && f.cvv) {
                ...
                defaultErrorFunction(c, "Missing Form Fields", 401);
            }
        };
    };
